...
def load_preprocess_data(limit=None):
    ...
    data[6] = data[6].apply(lambda x: [s.lower() for s in x])
    #remove punctuations
    data[6] = data[6].apply(lambda x: [re.sub(r'[^\w\s]', '', s) for s in x])
    # remove numbers
    # data[6] = data[6].apply(lambda x: [[sp.replace("[0-9]", " ") for sp in s] for s in x])
...
def make_elmo_embeddings(train, test, size=100):
    print("elmo is eating his cookie...")
    # split vector into smaller vectors for speed and ram manegment
    list_train = [train[i:i+size] for i in range(0,train.shape[0],size)]
    list_test = [test[i:i+size] for i in range(0,test.shape[0],size)]
    # Extract ELMo embeddings
    elmo_train = []
    elmo_test = []
    for i in range(len(list_train)):
        print("{}/{}".format(i, len(list_train)))
        elmo_train.append(elmo_vectors(list_train[i][6]))
    for i in range(len(list_test)):
        print("{}/{}".format(i, len(list_test)))
        elmo_test.append(elmo_vectors(list_test[i][6]))
    # elmo_train = [elmo_vectors(x[6]) for x in list_train]
    # elmo_test = [elmo_vectors(x[6]) for x in list_test]
    ...
    # save elmo_train_new
    pickle_out = open("elmo_embeddings/elmo_train_upsampled_v3_whole_julijan.pickle", "wb")
    pickle.dump(elmo_train_new, pickle_out)
    pickle_out.close()
    # save elmo_test_new
    pickle_out = open("elmo_embeddings/elmo_test_upsampled_v3_whole_julijan.pickle", "wb")
    pickle.dump(elmo_test_new, pickle_out)
    pickle_out.close()

def load_elmo_embeddings():
    # load elmo_train_new
    pickle_in = open("elmo_embeddings/elmo_train_upsampled_v3_whole_julijan.pickle", "rb")
    elmo_train_new = pickle.load(pickle_in)
    # load elmo_train_new
    pickle_in = open("elmo_embeddings/elmo_test_upsampled_v3_whole_julijan.pickle", "rb")
...
#-------MAIN------
train, test = load_preprocess_data(10000)
...
print(train.shape)
make_elmo_embeddings(train, test, 60)
